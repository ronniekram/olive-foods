...
export const Item = ({ name, price, description, sauces, servings, options }: ItemProps) => {
  return (
    <ItemWrap>
      <div tw="flex items-center justify-between text-base font-bold w-[90%]">
        <p>{name}</p>
        <p>{price && `$${price}`}</p>
      </div>
      <p tw="text-orange-200 text-xs font-semi">{servings && renderServings(servings)}</p>
      <p tw="text-sm">{description}</p>

      {sauces && (
        <div tw="text-xs leading-[1rem]">
          {sauces.map((s) => (
            <p key={s}>{s}</p>
          ))}
        </div>
      ...
      {options && (
        <div>
          <p tw="font-bold text-sm">{options.numChoices && `Choice of ${options.numChoices}`}</p>
          <List>
    ...
    <Page>
      <div tw="flex flex-col space-y-2">
        ...
        <div tw="w-full flex justify-between items-center font-sans text-grey">
          <p tw="text-sm font-bold">{minimum && `Minimum order: ${minimum}`}</p>
          <p tw="text-sm font-medium">Last updated: {lastUpdated}</p>
        </div>
        <div>
          {info &&
            info.map((txt) => (
              <p tw="text-xs text-grey font-sans" key={txt}>
                {txt}
              </p>
            ))}
        </div>
      </div>
      <div tw="flex flex-col space-y-8">{children}</div>
    </Page>
  </Wrapper>
);
